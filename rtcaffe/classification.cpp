...
int main(int argc, char** argv) {
  ...
  sprintf(CAM_CONFIG, CAM_CONFIG_TEMPLATE, CAM_WIDTH, CAM_HEIGHT, CAM_FRAMERATE);
  
  // Select Camera
  cv::VideoCapture cap;
  // Jetson Camera
  cap = cv::VideoCapture(CAM_CONFIG);
  ...
  for(;;)
  {
	...
	if(frame_num % 10 == 0){
		CHECK(!frame.empty()) << "Unable to decode image " << std::endl;
		caffe::Timer classifier_timer;
		classifier_timer.Start();
		std::vector<Prediction> predictions = classifier.Classify(frame);
		classifier_timer.Stop();
		std::cout << "elapsed time : " << classifier_timer.MilliSeconds() << "ms\n";
  	...
  	cv::imshow("BVLC Reference Net Real-Time Jetson TX1",frame);
